...
get_columns_for_model <- function(df,vol) {
  out <- df %>%
    mutate(symbol=factor(symbol),
           ...
           date_yday = yday(date)) %>%
    left_join(.,
              vol %>% select(date,open) %>% rename(volat_open = open),
              by = "date")
}
...
# get volatility, its kinda slow so dont do it during reruns
if (exists("dat_vix") == FALSE) {
  dat_vix <- tq_get(
    c(
      "^VIX"
      # ,"UNH"
